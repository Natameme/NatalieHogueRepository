...
const channel = Client.channels.cache.get('<870510476092641314>');
  //UTC Date
    const da = new Date();
    const dd = da.getDay();
...
//CONDITIONAL LOGIC
  //Trash
  if (cans == dayWeek.Day){
    channel.send("Today's Date is " + mdy );
    channel.send("Remember to Put out your cans! Trash Pickup is Tomorrow.")
  } else if (trashDay == dayWeek.Day) {
    channel.send("Today's Date is " + mdy );
    channel.send("Trash Pickup is Today! Remember to Bring in your cans this evening.")
  }
  //Sweep
  if (dayWeek.Day == 1){
        if (dayWeek.Week == 1 || dayWeek.Week == 3){
        channel.send("Today's Date is " + mdy );
        channel.send("Street Sweeping is Today! Make sure to move your car to the EVEN side of the street");
      } else if (dayWeek.Week == 2 || dayWeek.Week == 4){
        channel.send("Today's Date is " + mdy );
        channel.send("Street Sweeping is Today! Make sure to move your car to the ODD side of the street");
      }
  } else {
    channel.send("Today's Date is " + mdy );
  };
