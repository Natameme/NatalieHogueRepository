...
//Compile with:
//clang MidiPolyProto.c -o MidiProto -lportaudio -lportmidi
//Run with:
//./MidiProto

//clang MidiPolyProto.c -o MidiProto -lportaudio -lportmidi && ./MidiProto
...
void process(float *buffer, unsigned long numFrames, void *userData){
  ...
  for(unsigned long n=0; n < numFrames; n+=kNumChannels){
    ...
    clarFour = (0.5 *
                ...
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 2.0f  * 4.0f ) * 0.108f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 3.0f  * 4.0f ) * 0.0609f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 4.0f  * 4.0f ) * 0.0319f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 5.0f  * 4.0f ) * 0.0184f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 6.0f  * 4.0f ) * 0.0393f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 7.0f  * 4.0f ) * 0.0350f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 8.0f  * 4.0f ) * 0.0555f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 9.0f  * 4.0f ) * 0.0331f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 10.0f * 4.0f ) * 0.0412f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 11.0f * 4.0f ) * 0.0263f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 12.0f * 4.0f ) * 0.0151f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 13.0f * 4.0f ) * 0.0123f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 15.0f * 4.0f ) * 0.00237f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 16.0f * 4.0f ) * 0.00139f));
      //Fifteenth two
      fifTwo = (0.5 *
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 1.0f   * 8.0f ) * 0.116f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 2.0f   * 8.0f ) * 0.0223f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 3.0f   * 8.0f ) * 0.329f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 4.0f   * 8.0f ) * 0.00371f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 5.0f   * 8.0f ) * 0.000803f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 6.0f   * 8.0f ) * 0.000393f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 7.0f   * 8.0f ) * 0.00127f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 8.0f   * 8.0f ) * 0.00105f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 9.0f   * 8.0f ) * 0.000350f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 10.0f  * 8.0f ) * 0.000881f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 11.227 * 8.0f ) * 0.000489f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 12.0f  * 8.0f ) * 0.000323f));
      //melodia Eight
      melEight = (0.5 *
                 (sin(sineWave->phase[0] *  M_PI * 2.0f * 1.0f * 2.0f ) * 0.331f)+
                 (sin(sineWave->phase[0] *  M_PI * 2.0f * 2.0f * 2.0f ) * 0.0346f)+
                 (sin(sineWave->phase[0] *  M_PI * 2.0f * 3.0f * 2.0f ) * 0.0518f)+
                 (sin(sineWave->phase[0] *  M_PI * 2.0f * 4.0f * 2.0f ) * 0.00803f)+
                 (sin(sineWave->phase[0] *  M_PI * 2.0f * 5.0f * 2.0f ) * 0.0141f)+
                 (sin(sineWave->phase[0] *  M_PI * 2.0f * 6.0f * 2.0f ) * 0.00254f)+
                 (sin(sineWave->phase[0] *  M_PI * 2.0f * 7.0f * 2.0f ) * 0.00156f)+
                 (sin(sineWave->phase[0] *  M_PI * 2.0f * 8.0f * 2.0f ) * 0.00149f)+
                 (sin(sineWave->phase[0] *  M_PI * 2.0f * 10.0f * 2.0f) * 0.00102f));
      //Mixture 1 1/3
      mixOne = (0.5 *
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 1.0f    * 16.0f ) * 0.0421f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 1.331f  * 16.0f ) * 0.0588f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 2.0f    * 16.0f ) * 0.0623f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 2.662f  * 16.0f ) * 0.0117f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 3.0f    * 16.0f ) * 0.00291f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 4.0f    * 16.0f ) * 0.00436f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 5.0f    * 16.0f ) * 0.000588f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 5.325f  * 16.0f ) * 0.00467f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 6.0f    * 16.0f ) * 0.00184f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 6.657f  * 16.0f ) * 0.000582f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 7.0f    * 16.0f ) * 0.000158f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 8.0f    * 16.0f ) * 0.00358f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 9.0f    * 16.0f ) * 0.000239f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 10.0f   * 16.0f ) * 0.000616f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 10.651f * 16.0f ) * 0.00142f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 12.0f   * 16.0f ) * 0.000543f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 13.314  * 16.0f ) * 0.000518f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 14.0f   * 16.0f ) * 0.000327f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 14.645f * 16.0f ) * 0.000211f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 16.256f * 16.0f ) * 0.000803f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 18.0f   * 16.0f ) * 0.000254f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 18.641f * 16.0f ) * 0.000147f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 21.304f * 16.0f ) * 0.000285f)+
               (sin(sineWave->phase[0] *  M_PI * 2.0f * 23.967f * 16.0f ) * 0.000114f));
      //Octave Four
      octFour = (0.5 *
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 1.0f    * 4.0f )  * 0.221f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 2.0f    * 4.0f )  * 0.0749f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 3.0f    * 4.0f )  * 0.0165f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 4.0f    * 4.0f )  * 0.0113f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 5.0f    * 4.0f )  * 0.00254f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 6.0f    * 4.0f )  * 0.00316f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 7.0f    * 4.0f )  * 0.000653f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 8.0f    * 4.0f )  * 0.000350f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 9.0f    * 4.0f )  * 0.000478f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 10.0f   * 4.0f )  * 0.000668f)+
                (sin(sineWave->phase[0] *  M_PI * 2.0f * 11.0f   * 4.0f )  * 0.000741f)+
    ...
    float test = sin(sineWave->phase[0] * (2.0f * M_PI));
    ...
    for(int j = 1; j < kNumVoices; j++){

      buffer[n + c] = pow((sineWave->amplitude[0]+
                       sineWave->amplitude[1]+
                       sineWave->amplitude[2]+
                       sineWave->amplitude[3]), 0.25) * sine;

      sineWave->phase[1]  += sineWave->frequency[0];
      sineWave->phase[2]  += sineWave->frequency[1];
      sineWave->phase[3]  += sineWave->frequency[2];
      sineWave->phase[4]  += sineWave->frequency[3];

      sineWave->phase[0]  += sineWave->phase[1]+
                             sineWave->phase[2]+
                             sineWave->phase[3]+
                             sineWave->phase[4];
  ...
  if(Pm_Poll(sineWave->inputStream)){
    ...
    //Look for Note on message DEBUGGING
    ...
    //printf("velocity: %d \n---\n", Pm_MessageData2(sineWave->event.message));
      ...
      if(Pm_MessageStatus(sineWave->event.message) == 0x90){
        ...
        sineWave->frequency[v] = (55.0f * pow(2, (bag[v][0]- 69)/12.0f)) / kSamplingRate;//MTOF
      ...
      for(int i = 0; i < kNumVoices; i++){
          ...
          sineWave->frequency[i] = bag[i][1];//turn off freq
          sineWave->amplitude[i] = bag[i][1];//turn off note
        //  printf("v%i OFF note: %i    \nv%i velocity: %i\n--------\n", i, bag[i][0], i, bag[i][1]);
          } else {
          //do nothing
        //  printf("v%i INC note: %i    \nv%i velocity: %i\n--------\n", i, bag[i][0], i, bag[i][1]);
        }
      }
    }
      //bag[v][1] = 0;
