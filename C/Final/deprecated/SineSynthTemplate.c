...
//Compile with:
//clang SineSynthTemplate.c -o SawSynth -lportaudio -lportmidi
//Run with:
//./SawSynth

//clang SineSynthTemplate.c -o SawSynth -lportaudio -lportmidi && ./SawSynth
...
void process(float *buffer, unsigned long numFrames, void *userData){
  ...
  for(unsigned long n=0; n < numFrames; n+=kNumChannels){
    ...
    // per sine structure (sin(sineWave->phase * M_PI * 2.0f * harmonic * octave ) * amplitude per harmonic)
    // Pi * 2 == tau
    // Octave/Stop Length, translation between feet and mult (16'=1.0f 8'=2.0f 4'=4.0f 2'=8.0f 1'=16.0f)
    // TODO: Allow Per-Stop CC control for blending
    ...
    */
    int v = kNumVoices;
      ...
      /*WHEN NOTE ON
        ...
        1. get note|velocity pairs (Pm_MessageData1 and 2 respectively)
        2. store pair in current round-robin slot
        1. increment slot number
      */
        v++;//1. increment to new slot
        ...
        sineWave->frequency = (55.0f * pow(2, (bag[v][0]- 69)/12.0f)) / kSamplingRate;//MTOF
        sineWave->amplitude = bag[v][1] / 127.0f;//Velocity2Amplitude
      ...
      /*WHEN NOTE OFF
        1. get note number
        2. find slot which equals note number
        3. zero out slot
